...
void FIRRTLTypesLowering::visitDecl(MemOp op) {
  ...
  auto maybeFlip = [](Type a) -> std::tuple<BundleType, bool> {
    return TypeSwitch<Type, std::tuple<BundleType, bool>>(a)
  ...
  for (auto field : fieldTypes) {
      ...
      for (auto elt : portType.getElements())
        resultFields.push_back(
            StringSwitch<BundleType::BundleElement>(elt.name)
                .Cases("data", "rdata", "wdata", {elt.name, field.type})
                .Cases("mask", "wmask",
    ...
    for (size_t i = 0, e = op.getNumResults(); i != e; ++i) {
      ...
      for (auto elt : portType.getElements()) {
        ...
        case 2: {
          FIRRTLType theType = field.type.cast<FIRRTLType>();
          if (isFlipped)
            theType = FlipType::get(theType);
